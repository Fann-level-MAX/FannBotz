...
┊-------------------------------------
┊• *PremiumTime:*
${sortedP.slice(0, len).map(({ jid, name, premiumTime, registered }, i) => `${premTime > 0 ? `${clockString (premTime - new Date() * 1)}` : '┊ *EXPIRED 🚫*'}`).join`\n┗━═┅═━––––––๑`}
...
handler.tags = ['info']
handler.command = /^(listprem|premlist|myprem)$/i
